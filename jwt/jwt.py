...
class JWTHelper():
    ...
    @classmethod
    def generateJWT(cls, principalName, name, groups):
    ...
    @classmethod
    def generateUserJWTFromLDAPData(cls, userLDAPData):
    ...
    def validateUserJWT(cls, token):
        ...
        try:
            claims_options = { 'iss' : { 'essential' : True, 'values' : [cls.JWT_ISSUER]}, 'exp' : { 'essential' : True, 'validate' : cls.jwtExpiryOK }}
            ...
            # validatation will raise exception on any errors - if we got this far we are ok
            return True
        except:
            return False
